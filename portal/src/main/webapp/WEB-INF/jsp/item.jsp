	 ...
	 <div class="seller hide">
				...
				<li id='choose-type'></li>
					...
					<div class="dd">
						<div class="wrap-input">
								<a class="btn-reduce" href="javascript:;" onclick="reduce('#buy-num')">减少数量</a>
								<a class="btn-add" href="javascript:;" onclick="add('#buy-num')">增加数量</a>
		...
		<div id="preview">
			<div id="spec-n1" class="jqzoom" clstag="shangpin|keycount|product|spec-n1">
				<img id="main-img" data-img="1" width="350" height="350" src="${item.images[0]}" alt="${item.title}"  jqimg="${item.images[0]}"/>
			</div>
...
<div class="w" >
	<div class="right">
		<div id="product-detail" class="m m1" data-widget="tabs" clstag="shangpin|keycount|product|detail">
			<div class="mt">
				<ul class="tab">
					<li id="con-attr1" clstag="shangpin|keycount|product|pinfotab" data-widget="tab-item" class="curr"><a id="p-con-attr1" href="javascript:;" >商品介绍</a></li>
					<li id="con-attr2" clstag="shangpin|keycount|product|pcanshutab" data-widget="tab-item"><a id="p-con-attr2" href="javascript:;" >规格参数</a></li>
				</ul>
			</div>
...
<script type="text/javascript">
	var itemControl = {
			...
			getItemParam:function(itemId) {
                ...
                $("#product-detail-2").show();
                $("#detail-list").hide();
                $("#con-attr1").attr("class","");
                $("#con-attr2").attr("class","curr");
			}
	};
	$(function(){
		//取商品id
		var itemId = "${item.id}";
		//给商品规格参数tab页绑定事件
		$("#p-con-attr2").bind("click", function(){

            itemControl.getItemParam(itemId);
        });
        $("#p-con-attr1").bind("click", function(){
            $("#con-attr1").attr("class","curr");
            $("#con-attr2").attr("class","");
            $("#product-detail-2").hide();
            $("#detail-list").show();
	...
	$("#spec-forward").click(function(){
		...
		if(prev[0]){
			...
			prev.attr("class","hover");
			prev.children().attr("class","img-hover");
            $("#main-img").attr("src", prev.children().attr("src"))
		}
	});
    $("#spec-backward").click(function(){
        ...
        if(next[0]){
            $(".hover").attr("class","");
            $(".img-hover").attr("class","");
            next.attr("class","hover");
            next.children().attr("class","img-hover");

            $("#main-img").attr("src", next.children().attr("src"))

        }
    });
 		 //加减商品
        function add(id){
            var num = Number($(id).val())+1;
            $(id).val(num);
        }
		function reduce(id){
			if ($(id).val()>1){
